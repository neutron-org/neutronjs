import { GeneratedType, Registry, OfflineSigner } from "@cosmjs/proto-signing";
import { AminoTypes, SigningStargateClient } from "@cosmjs/stargate";
import { HttpEndpoint } from "@cosmjs/tendermint-rpc";
import * as gaiaGlobalfeeV1beta1TxAmino from "./globalfee/v1beta1/tx.amino.js";
export declare const gaiaAminoConverters: {
    "/gaia.globalfee.v1beta1.MsgUpdateParams": {
        aminoType: string;
        toAmino: ({ authority, params }: import("./globalfee/v1beta1/tx.js").MsgUpdateParams) => gaiaGlobalfeeV1beta1TxAmino.MsgUpdateParamsAminoType["value"];
        fromAmino: ({ authority, params }: gaiaGlobalfeeV1beta1TxAmino.MsgUpdateParamsAminoType["value"]) => import("./globalfee/v1beta1/tx.js").MsgUpdateParams;
    };
};
export declare const gaiaProtoRegistry: ReadonlyArray<[string, GeneratedType]>;
export declare const getSigningGaiaClientOptions: ({ defaultTypes, }?: {
    defaultTypes?: ReadonlyArray<[string, GeneratedType]>;
}) => {
    registry: Registry;
    aminoTypes: AminoTypes;
};
export declare const getSigningGaiaClient: ({ rpcEndpoint, signer, defaultTypes, }: {
    rpcEndpoint: string | HttpEndpoint;
    signer: OfflineSigner;
    defaultTypes?: ReadonlyArray<[string, GeneratedType]>;
}) => Promise<SigningStargateClient>;
