...
class MsgClientImpl {
    ...
    execLegacyContent(request) {
        const data = tx_js_1.MsgExecLegacyContent.encode(request).finish();
        const promise = this.rpc.request("cosmos.gov.v1.Msg", "ExecLegacyContent", data);
        return promise.then((data) => tx_js_1.MsgExecLegacyContentResponse.decode(new binary_js_1.BinaryReader(data)));
    }
    vote(request) {
    ...
    voteWeighted(request) {
        const data = tx_js_1.MsgVoteWeighted.encode(request).finish();
        const promise = this.rpc.request("cosmos.gov.v1.Msg", "VoteWeighted", data);
        return promise.then((data) => tx_js_1.MsgVoteWeightedResponse.decode(new binary_js_1.BinaryReader(data)));
    }
    deposit(request) {
    ...
    cancelProposal(request) {
        const data = tx_js_1.MsgCancelProposal.encode(request).finish();
        const promise = this.rpc.request("cosmos.gov.v1.Msg", "CancelProposal", data);
        return promise.then((data) => tx_js_1.MsgCancelProposalResponse.decode(new binary_js_1.BinaryReader(data)));
    }
}
