...
class MsgClientImpl {
    ...
    fundTreasury(request) {
        const data = tx_js_1.MsgFundTreasury.encode(request).finish();
        const promise = this.rpc.request("neutron.revenue.Msg", "FundTreasury", data);
        return promise.then((data) => tx_js_1.MsgFundTreasuryResponse.decode(new binary_js_1.BinaryReader(data)));
    }
}
