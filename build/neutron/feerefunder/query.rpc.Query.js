...
class QueryClientImpl {
    ...
    feeInfo(request) {
        const data = query_js_1.FeeInfoRequest.encode(request).finish();
        const promise = this.rpc.request("neutron.feerefunder.Query", "FeeInfo", data);
        return promise.then((data) => query_js_1.FeeInfoResponse.decode(new binary_js_1.BinaryReader(data)));
    }
}
