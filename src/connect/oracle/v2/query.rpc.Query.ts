...
export class QueryClientImpl implements Query {
  ...
  getAllCurrencyPairs(request: GetAllCurrencyPairsRequest = {}): Promise<GetAllCurrencyPairsResponse> {
    ...
    const promise = this.rpc.request("connect.oracle.v2.Query", "GetAllCurrencyPairs", data);
  ...
  getPrice(request: GetPriceRequest): Promise<GetPriceResponse> {
    ...
    const promise = this.rpc.request("connect.oracle.v2.Query", "GetPrice", data);
    return promise.then((data) => GetPriceResponse.decode(new BinaryReader(data)));
  }
  getPrices(request: GetPricesRequest): Promise<GetPricesResponse> {
    const data = GetPricesRequest.encode(request).finish();
    const promise = this.rpc.request("connect.oracle.v2.Query", "GetPrices", data);
  ...
  getCurrencyPairMapping(
    ...
    const promise = this.rpc.request("connect.oracle.v2.Query", "GetCurrencyPairMapping", data);
