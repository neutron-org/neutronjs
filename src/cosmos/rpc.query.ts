...
export const createRPCQueryClient = async ({ rpcEndpoint }: { rpcEndpoint: string | HttpEndpoint }) => {
  ...
  return {
    cosmos: {
      adminmodule: {
        adminmodule: (await import("./adminmodule/adminmodule/query.rpc.Query")).createRpcQueryExtension(
          client,
        ),
      },
      app: {
      ...
      base: {
        ...
        tendermint: {
          v1beta1: (await import("./base/tendermint/v1beta1/query.rpc.Service")).createRpcQueryExtension(
            client,
          ),
        },
      },
