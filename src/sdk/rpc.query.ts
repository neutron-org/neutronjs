...
export const createRPCQueryClient = async ({ rpcEndpoint }: { rpcEndpoint: string | HttpEndpoint }) => {
  ...
  return {
    cosmos: {
      adminmodule: {
        adminmodule: (
          await import("../cosmos/adminmodule/adminmodule/query.rpc.Query")
      ...
      base: {
        node: {
          v1beta1: (await import("../cosmos/base/node/v1beta1/query.rpc.Service")).createRpcQueryExtension(
            client,
          ),
        },
        tendermint: {
          v1beta1: (
            await import("../cosmos/base/tendermint/v1beta1/query.rpc.Service")
          ).createRpcQueryExtension(client),
        },
      },
      circuit: {
    ...
    sdk: {
      auction: {
        v1: (await import("./auction/v1/query.rpc.Query")).createRpcQueryExtension(client),
      },
      mempool: {
        v1: (await import("./mempool/v1/query.rpc.Service")).createRpcQueryExtension(client),
      },
    },
  };
};
