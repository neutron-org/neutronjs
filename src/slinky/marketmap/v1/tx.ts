...
function createBaseMsgRemoveMarkets(): MsgRemoveMarkets {
  return {
    authority: "",
    markets: [],
  };
}
export const MsgRemoveMarkets = {
  typeUrl: "/slinky.marketmap.v1.MsgRemoveMarkets",
  encode(message: MsgRemoveMarkets, writer: BinaryWriter = BinaryWriter.create()): BinaryWriter {
    if (message.authority !== "") {
      writer.uint32(10).string(message.authority);
    }
    for (const v of message.markets) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): MsgRemoveMarkets {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  fromJSON(object: any): MsgRemoveMarkets {
    const obj = createBaseMsgRemoveMarkets();
    if (isSet(object.authority)) obj.authority = String(object.authority);
    if (Array.isArray(object?.markets)) obj.markets = object.markets.map((e: any) => String(e));
    return obj;
  },
  toJSON(message: MsgRemoveMarkets): JsonSafe<MsgRemoveMarkets> {
    const obj: any = {};
    message.authority !== undefined && (obj.authority = message.authority);
    if (message.markets) {
      obj.markets = message.markets.map((e) => e);
    } else {
      obj.markets = [];
    }
    return obj;
  },
  fromPartial<I extends Exact<DeepPartial<MsgRemoveMarkets>, I>>(object: I): MsgRemoveMarkets {
    const message = createBaseMsgRemoveMarkets();
    message.authority = object.authority ?? "";
...
function createBaseMsgRemoveMarketsResponse(): MsgRemoveMarketsResponse {
  return {
    deletedMarkets: [],
  };
}
export const MsgRemoveMarketsResponse = {
  typeUrl: "/slinky.marketmap.v1.MsgRemoveMarketsResponse",
  encode(message: MsgRemoveMarketsResponse, writer: BinaryWriter = BinaryWriter.create()): BinaryWriter {
    for (const v of message.deletedMarkets) {
      writer.uint32(10).string(v!);
    }
    return writer;
  },
  decode(input: BinaryReader | Uint8Array, length?: number): MsgRemoveMarketsResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  fromJSON(object: any): MsgRemoveMarketsResponse {
    const obj = createBaseMsgRemoveMarketsResponse();
    if (Array.isArray(object?.deletedMarkets))
      obj.deletedMarkets = object.deletedMarkets.map((e: any) => String(e));
    return obj;
  },
  toJSON(message: MsgRemoveMarketsResponse): JsonSafe<MsgRemoveMarketsResponse> {
    const obj: any = {};
    if (message.deletedMarkets) {
      obj.deletedMarkets = message.deletedMarkets.map((e) => e);
    } else {
      obj.deletedMarkets = [];
    }
    return obj;
  },
  fromPartial<I extends Exact<DeepPartial<MsgRemoveMarketsResponse>, I>>(
    object: I,
